    ...
    private String buildScope(User user) {
        ...
        if (!CollectionUtils.isEmpty(user.getRoles())) {
            user.getRoles().forEach(role -> {
                stringJoiner.add(
                        "ROLE_" + role.getName()); // thêm prefix ROLE_ để phân biệt với Permission. Vì khi generate
                // token. ROLE và permission đều được gán như nhau.
    ...
    public void logout(LogoutRequest request) throws ParseException, JOSEException {
        try {
            var signToken = verifyToken(
                    request.getToken(),
                    true); // true giống lúc refresh bởi vì để đảm bảo token Client đã logout sẽ bị hêt hạn (lưu DB
            // invalidated_token). và token đã logout ko thể sử dụng ở nơi khác
